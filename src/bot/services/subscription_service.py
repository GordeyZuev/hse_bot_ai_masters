...
class SubscriptionService:
    ...
    async def get_user_subscriptions(self, user_id: int) -> List[Subject]:
        ...
        async with db_manager.async_session() as session:
            try:
                stmt = select(Subject).join(Subscription).where(
    ...
    async def subscribe_user(self, user_id: int, subject_id: int) -> Tuple[bool, str]:
        ...
        async with db_manager.async_session() as session:
            try:
                ...
                logger.error(f"Ошибка подписки пользователя {user_id} на предмет {subject_id}: {e}")
    ...
    async def unsubscribe_user(self, user_id: int, subject_id: int) -> Tuple[bool, str]:
        ...
        async with db_manager.async_session() as session:
            try:
                ...
                await session.rollback()
                logger.error(f"Ошибка отписки пользователя {user_id} от предмета {subject_id}: {e}")
    ...
    async def unsubscribe_user_from_all(self, user_id: int) -> Tuple[bool, str]:
        ...
        async with db_manager.async_session() as session:
            try:
                ...
                await session.rollback()
                logger.error(f"Ошибка отписки пользователя {user_id} от всех предметов: {e}")
    ...
    async def get_subscription_stats(self) -> dict:
        ...
        async with db_manager.async_session() as session:
            try:
                ...
                ).join(Subscription).group_by(Subject.id, Subject.name).order_by(
                    func.count(Subscription.subject_id).desc()
                ).limit(5)
                result = await session.execute(stmt)
