...
#### Handlers
- **admin.py** - Административные функции (статистика, рассылка, мгновенная синхронизация)
...
#### Services
- **admin_service.py** - Административные операции (статистика, рассылка, логи)
...
1. **Планировщик** запускает задачу синхронизации каждый час
...
1. **Администратор** нажимает кнопку "🔄 Синхронизация" в админ-панели
2. **admin.py** обработчик `callback_admin_sync` вызывает `data_syncer.sync_data()`
3. Выполняется немедленная синхронизация данных
4. Администратор получает уведомление о результате операции
